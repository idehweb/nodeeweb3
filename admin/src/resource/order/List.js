                ...
                {isSmall ? (
                    <SimpleList
                      ...
                      /></span>
                        ...
                        secondaryText={record => <div className={'d-dfgfd'}><span>{dateFormat(record.createdAt)}</span><span>{record?.amount?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' ' + t(themeData.currency)}</span></div>}
