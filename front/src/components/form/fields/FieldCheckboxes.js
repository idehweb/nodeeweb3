...
function FieldCheckboxes({ field }) {
  ...
  const removeChild = (id) => {
    let temp = theArray.filter((i, idx) => id !== idx);
    let tempVal = value.filter((i, idx) => id !== idx);
    setArray(temp);
    console.log('name:', name);
  ...
  return (
    <Col
      ...
      <Field name={name} style={style} className="mb-2 form-control">
        {() => {
    ...
    <div className={'max-width100'}>
      ...
      <div className={'width-more'}>
        ...
        {typeof input.value == 'object' &&
          Object.keys(theVal[theKey]).map((theKey2, inx) => {
