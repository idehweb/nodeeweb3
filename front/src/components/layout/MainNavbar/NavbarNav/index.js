        ...
        children.map((ch, chx) => {
          ...
          return (
            <NavItem key={chx}>
              ...
              {ch.children && (
                <ChildItem setStyles={setStyles} children={ch.children} />
              )}
            </NavItem>
          );
        })}
...
export default () => {
  ...
  return (
    <Nav navbar className={'flex-row top-bar-menu stfwrap ' + searchform}>
      ...
      {selectedCats &&
        selectedCats.length &&
        selectedCats
          .orderBy((h) => parseFloat(h.order))
          .map((sc, key) => {
            return (
              <CustomNavItem
                href={'/category/' + sc._id + '/' + sc.name.fa}
                text={sc.name.fa}
                key={key}>
                {sc.children && (
                  <ul className={'children'}>
                    {sc.children.map((sc1, key2) => {
                      return (
                        <CustomNavItem
                          className={'theChild'}
                          href={'/category/' + sc1._id + '/' + sc1.name.fa}
                          text={sc1.name.fa}
                          key={key2}>
                          {sc1.children && (
                            <ul className={'children'}>
                              {sc1.children.map((sc2, key3) => {
                                return (
                                  <CustomNavItem
                                    className={'theChild'}
                                    href={
                                      '/category/' + sc2._id + '/' + sc2.name.fa
                                    }
                                    text={sc2.name.fa}
                                    key={key3}></CustomNavItem>
                                );
                              })}
                            </ul>
                          )}
