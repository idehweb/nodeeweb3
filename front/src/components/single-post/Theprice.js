...
const Theprice = (props) => {
  ...
  if (combinations && combinations[0]) {
    ...
    let lessPrice = null;
    if (
      combinations[0].price &&
      combinations[0].price != '' &&
      combinations[0].price != null
    )
      lessPrice = combinations[0].price;
    if (
      combinations[0].salePrice &&
      combinations[0].salePrice != '' &&
      combinations[0].salePrice != null
    )
  ...
  if (themeData.tax && themeData.taxAmount) {
    ...
    // 9/100 * price + price
    // ((0.09)+ 1) * price
    if (price) price = parseInt((ta / 100 + 1) * parseInt(price));
    if (salePrice) salePrice = parseInt((ta / 100 + 1) * parseInt(salePrice));
    // price=ta;
    //     salePrice=ta;
  ...
  return (
    <div className={'thePrice rtl ' + className}>
      ...
      <div className={'with-sale-price'}>
        {Boolean(salePrice && salePrice !== null) && (
