    ...
    <Container fluid className="main-content-container fghjkjhgf">
      <Row className="relative mt-3 mb-3">
        <Col>
          {!enableAddContact && (
            <Button
              onClick={enableAddContactfunc}
              className={'fixed buttop lefter'}>
              <PersonAddIcon />
            </Button>
          )}
          {enableAddContact && (
            <form onSubmit={addContact}>
              <input
                type={'tel'}
                className={'fixed customer_f'}
                onChange={(e) => setPhoneNumber(e.target.value)}
                value={phoneNumber}
              />
              <Button onClick={addContact} className={'fixed buttop lefter'}>
                <PersonAddIcon />
              </Button>
            </form>
          )}
      ...
      {contacts &&
        contacts.map((e, k) => {
          return (
            <Row className="relative mt-3 mb-3" key={k}>
              <Col></Col>
            </Row>
          );
        })}
