...
const Entities = (props) => {
  ...
  const loadProductItems = async (page, filter = {}) => {
    ...
    await setInitialLoad(false);
    await setLoadingMoreItems(true);
    getEntities(params._entity, newOffset, 24, search || '', filter).then(
      (resp) => {
        setLoadingMoreItems(false);
        afterGetData(resp);
      }
  ...
  if (isClient) {
    ...
    if (attr !== attrP) setAttr(attr);
    if (value !== valueP) setValue(value);
  }
  // useEffect(() => {
  ...
  if (catid || search) showSlide = false;
              ...
              {tracks &&
                tracks.map((i, idxx) => (
                  <Col
                    ...
                    sm="4"
                    xs="6"
                    className={'nbghjk post-style-' + postCardMode}>
