...
let self = ({
  ...
  getAll: function (req, res, next) {
    ...
    // return res.json(Item.schema.paths);
    // console.log("Item.schema => ",Item.schema.paths);
    ...
    _.forEach(tt, (item) => {
      ...
      if (Item.schema.paths[item]) {
        ...
        // console.log("instance of item ===> ",Item.schema.paths[item].instance);
    ...
    if (search['productCategory.slug']) {
      ...
      ProductCategory.findOne({slug: search['productCategory.slug']}, function (err, productcategory) {
        ...
        if (productcategory._id) {
          ...
          Item.find(ss, function (err, products) {

            Item.countDocuments(ss, function (err, count) {
              if (err || !count) {
                res.json([]);
                return 0;
              }
      ...
      q = Item.find(search, fields).populate('productCategory', '_id slug').skip(offset).sort(sort).limit(parseInt(req.params.limit));
  ...
  importFromLatest: function (req, res, next) {
    ...
    let filePath = path.join(__dirname, "./entity/item/", "arzesh.csv");
    let Item = req.mongoose.model('Item');

// console.log('filePath',filePath)
    let data = readFile(filePath, "utf8").then((items) => {
      console.log('item', typeof items)
      let csvFile = items.split("\n");
      _.forEach(csvFile, (itm) => {
        let row = itm.split(",");
        let v={
          'code':row[0],
          'title':row[1],
