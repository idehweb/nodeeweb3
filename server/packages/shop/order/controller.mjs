...
const self = {
  all: function (req, res, next) {
    ...
    Order.find(
      search,
      "_id , orderNumber , customer_data , customer , sum , amount , paymentStatus , status , createdAt , updatedAt , card",
      function (err, orders) {
        if (err || !orders) {
          console.log("err", err);
          res.json([]);
  ...
  createByCustomer: function (req, res, next) {
    ...
    _forEach(req.body.card, function (pack) {
      ...
      Product.findOne(
        ...
        function (err, ps) {
          ...
          if (ps.type != "normal") {
            if (ps.combinations) {
              _forEach(ps.combinations, function (comb, inde) {
              ...
              .then(function (resp) {
                ...
                Settings.findOne({}, "tax taxAmount", function (err, setting) {
                  ...
                  function update_order() {
                    console.log("\t\twe are herersdffd3");
                    // console.log('==> update_order')
                    if (req.body.order_id && req.body.order_id.length) {
                  ...
                  function change_products_quantities() {
                    ...
                    // _forEach(tempProducts, function (tempProduct) {
                    //     console.log('\ntempProduct',{
  ...
  create: function (req, res, next) {
    ...
    _forEach(req.body.card, function (pack) {
      ...
      Product.findOne(
        { _id: id },
        "_id combinations type price salePrice title",
        function (err, ps) {
          // console.log('found id:', id, 'main_id[1]:', main_id[1], 'ps', ps);
          ii++;
          if (ps.type != "normal") {
  ...
  createAdmin: function (req, res, next) {
    ...
    if (req.body.card) {
      ...
      _forEach(req.body.card, function (item, i) {
  ...
  checkOrder: function (req, item, k = -1) {
    ...
    if (item.data && item.data.coupon_lines) {
      _forEach(item.data.coupon_lines, (coupon_lines) => {
    ...
    if (item.data && item.data.line_items) {
      let theCart = [],
        thePackage = [];
      _forEach(item.data.line_items, (cart_data) => {
        theCart.push({
          _id: cart_data.id,
          sku: cart_data.sku,
    ...
    if (item.data && item.data.meta_data && item.data.meta_data[0]) {
      _forEach(item.data.meta_data, (j) => {
