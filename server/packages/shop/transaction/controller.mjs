...
var self = {
    ...
    buy: function (req, res, next) {
        ...
        if (
            req.params._price &&
            (req.params._price == null || req.params._price == "null")
        )
            return res.json({
                success: false,
        ...
        Settings.findOne({}, "currency", function (err, setting) {
            ...
            if (req.body.method)
                Gateway.findOne({ slug: req.body.method }, function (err, gateway) {
                    ...
                    Order.findById(
                        req.params._id,
                        "sum , orderNumber , amount , discount , customer",
                        function (err, order) {
                            if (err || !order) {
                                res.json({
                                    success: false,
                            ...
                            console.log("gateway.request last", theReq);
                            ...
                            req
                                ...
                                .then(function (parsedBody) {
                                    ...
                                    Transaction.create(obj, function (err, transaction) {
                                        ...
                                        req.fireEvent(
                                            "create-transaction-by-customer",
                                            transaction
                                        );
                        ...
                        if (parsedBody.result == 100) {
                                ...
                                function (err, transaction) {
                                    ...
                                    Order.findByIdAndUpdate(
                                        ...
                                        function (err, order) {
                                            ...
                                            if (
                                                order.deliveryDay &&
                                                order.deliveryDay.theid == "chapar"
                                            ) {
                                                let sumTitles = "",
                                                    theTotal = 0;
                                                ...
                                                let string =
                                                    ...
                                                    '","service": "1","value": "' +
                                                    theTotal +
                                                    '","payment_term": 0,"weight": "1","content":"' +
                                                    sumTitles +
                                                    '","change_state_url":""},"sender": {"person": "حسین محمدی","company": "شرکت گارانتی آروند","city_no": "10866","telephone": "+982142528000","mobile": "989024252802","email": "info@localhost:3001","address": "تهران، کاووسیه، بلوار میرداماد، پلاک ۴۹۶، مجتمع پایتخت، بلوک A، طبقه ۹، واحد ۹۰۱","postcode": "1969763743"},"receiver": {"person": "' +
                                                    (order.customer.firstName +
                                                        " " +
                                                        order.customer.lastName) +
                                                    '","company": "","city_no": "' +
                                                    order.billingAddress.City_no +
                                                    '","telephone": "' +
                                                    order.billingAddress.PhoneNumber +
                                                    '","mobile": "' +
                                                    order.customer.phoneNumber +
                                                    '","email": "test@test.com","address": "' +
                            ...
                            Transaction.findByIdAndUpdate(
                                transaction._id,
                                {
                                    ...
                                    status:
                                        req.body.Status == 1 || req.body.Status == 2 ? true : false,
                                    updatedAt: new Date(),
                                },
                                function (err, transaction) {
                                    if (err || !transaction) {
    ...
    verify: function (req, res, next) {
        ...
        Transaction.findOne(
            { Authority: req.params.bank_token },
            function (err, transaction) {
                if (err || !transaction) {
                    res.json({
                        success: false,
