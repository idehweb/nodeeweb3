...
export default (props) => {
        ...
        if (settings && settings.defaultSmsGateway) {
          ...
          Gateway.findById(settings.defaultSmsGateway, function (err, gateway) {
            ...
            if (phoneNumber) {
              ...
              if (phoneNumber)
                m = m.replaceAll("%phoneNumber%", phoneNumber);
              console.log('m:', m)
  ...
  function sendToSms(req, status, params) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      Settings.findOne({}, 'plugins currency', function (err, setting) {
        ...
        if (!setting.plugins['sms-gateway'])
          return reject({})


        let {onCreateOrderByCustomer, onUpdateTransactionByCustomer, onScheduleTask, onBackupComplete, onCustomerRegister,onCustomerSubmitFirstName} = setting.plugins['sms-gateway'];
        ...
        if (status == 'backup-completion') {
          ...
          publishToSms(req, phoneNumber, onBackupComplete || 'back up completed')


        }
        if (status == 'create-customer' && onCustomerRegister) {
          onCustomerRegister = washString(onCustomerRegister, {customer_data: params});
          // console.log('customer register4...',params)
          publishToSms(req, phoneNumber, onCustomerRegister || 'customer registered')


        }
        if (status == 'create-customer-first-name' && onCustomerSubmitFirstName) {
          onCustomerSubmitFirstName = washString(onCustomerSubmitFirstName, {customer_data: params});
          console.log('create-customer-first-name',params)
          publishToSms(req, phoneNumber, onCustomerSubmitFirstName || 'customer registered')
  ...
  props['plugin']['sms-gateway'] = [
    ...
    {name: "onCustomerRegister", value: '', label: "onCustomerRegister", type: "textarea"},
    {name: "onCustomerSubmitFirstName", value: '', label: "onCustomerSubmitFirstName", type: "textarea"},
  ];
  return props;
}
