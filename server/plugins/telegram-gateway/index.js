  ...
  function sendToTelegram(req, status, params) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      Settings.findOne({}, 'plugins currency', function (err, setting) {
        ...
        let {onCreateOrderByCustomer, onUpdateTransactionByCustomer, onScheduleTask, telegramLink, telegramChatID, onBackupComplete, onCustomerRegister} = setting.plugins['telegram-gateway'];
        ...
        if (status == 'create-customer') {
          ...
          publishToTelegram(req, telegramLink, telegramChatID, onCustomerRegister || 'customer registered')
        ...
        if (item.routes)
          item.routes.push({
            ...
            "controller": (req, res, next) => {
              ...
              Settings.findOne({}, 'plugins currency', function (err, setting) {
                console.log('setting:', setting)
                if (!setting || !setting.plugins || !setting.plugins['telegram-gateway']) {
                  return res.json({})
                }
                let {onScheduleTask, telegramLink, telegramChatID} = setting.plugins['telegram-gateway'];
                getReport(req, onScheduleTask).then((message) => {
